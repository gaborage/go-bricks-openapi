...
func bindValueSpec(scope map[string]binding, spec ast.Spec, isConst bool) {
	...
	for i, name := range valueSpec.Names {
		b := binding{from: valueSpec.End()}
		if isConst && i < len(valueSpec.Values) {
			b.value, b.ok = stringLiteralValue(valueSpec.Values[i])
		}
		bind(scope, name, b)
	}
}
...
// stringLiteralValue returns the value of a string-literal expression. The bool
// result — not an empty return value — reports whether the expression was a
// string literal at all: `const prefix = ""` is a legitimate, resolvable empty
// prefix, and conflating it with "not a string" would drop every route built
// on it.
