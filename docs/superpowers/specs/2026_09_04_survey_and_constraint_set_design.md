...
- The `verdictUnreadable` divergence (generate passes, doctor fails) is preserved: it never sets `Warned`, and doctor fails it in pre-flight before any Survey. Aligning it is a separate `fix`.
